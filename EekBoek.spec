...
%if %{gui}
%package gui
Summary: GUI enhancements for %{pkgname}.
Group: Applications/Productivity
AutoReqProv: 0
Requires: %{name} = %{version}
...
%define ebconf  %{_sysconfdir}/%{lcpkg}
%define ebshare %{_datadir}/%{pkgname}%{pkgsuffix}-%{version}

%{__rm} -rf $RPM_BUILD_ROOT

%{__mkdir_p} ${RPM_BUILD_ROOT}%{ebconf}
%{__install} -m 0644 examples/%{lcname}.conf ${RPM_BUILD_ROOT}%{ebconf}/%{lcpkg}.conf
...
echo "#!%{__perl}" > ${RPM_BUILD_ROOT}%{_bindir}/ebshell%{lcpkgsuffix}

sed -s "s;# use lib qw(EekBoekLibrary;use lib qw(%{ebshare}/lib;" \
	< script/ebshell >> ${RPM_BUILD_ROOT}%{_bindir}/ebshell%{lcpkgsuffix}
%{__chmod} 0755 ${RPM_BUILD_ROOT}%{_bindir}/ebshell%{lcpkgsuffix}

cat <<EOD > .files
%defattr(-,root,root)
%doc CHANGES README INSTALL QUICKSTART examples doc/html TODO
%dir %{_sysconfdir}/%{lcpkg}
%config(noreplace) %{_sysconfdir}/%{lcpkg}/%{lcpkg}.conf
%dir %{ebshare}
%{_bindir}/ebshell%{lcpkgsuffix}
%{_mandir}/man1/*
EOD
...
pod2man blib/script/ebshell > ${RPM_BUILD_ROOT}%{_mandir}/man1/ebshell%{lcpkgsuffix}.1
