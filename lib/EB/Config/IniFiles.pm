...
EB::Config::IniFiles - A module for reading .ini-style configuration files.

=head1 SYNOPSIS

  use EB::Config::IniFiles;
  my $cfg = new EB::Config::IniFiles( -file => "/path/configfile.ini" );
...
EB::Config::IniFiles provides a way to have readable configuration files outside
...
Get a new EB::Config::IniFiles object with the I<new> method:

  $cfg = EB::Config::IniFiles->new( -file => "/path/configfile.ini" );
  $cfg = new EB::Config::IniFiles -file => "/path/configfile.ini";
...
file has an error).  One EB::Config::IniFiles object is required per configuration
...
is none, EB::Config::IniFiles will look to your default section for a "permissions"
...
EB::Config::IniFiles object. When importing settings from another object, 
...
sub new {
  ...
  $self->{cf}        = undef;
  if( ref($parms{-import}) && ($parms{-import}->isa('EB::Config::IniFiles')) ) {
...
If an error occurs while parsing the INI file the @EB::Config::IniFiles::errors
...
#  $@ = ''; $! = undef;  # Clear any error messages
  ...
  while ( @lines ) {
	...
	if ($foundeot) {
	  ...
	  push(@EB::Config::IniFiles::errors, sprintf('%d: %s', $startline,
	...
	if (exists $self->{v}{$sect}{$parm} &&
      ...
      push(@EB::Config::IniFiles::errors, sprintf("Line \%d in file " . $self->{cf} . " is mal-formed:\n\t\%s", $lineno, $_));
    }
  }
  ...
  foreach( keys %{$self->{v}} ) {
    ...
    tie %{$self->{v}{$_}}, 'EB::Config::IniFiles::_section', %parms
  } # end foreach

  @EB::Config::IniFiles::errors ? undef : 1;
}
...
If you created the EB::Config::IniFiles object without initialising from
...
=head2 tie %ini, 'EB::Config::IniFiles', (-file=>$filename, [-option=>value ...] )

Using C<tie>, you can tie a hash to a B<EB::Config::IniFiles> object. This creates a new
...
Here's an example:
  ...
  my %ini
  tie %ini, 'EB::Config::IniFiles', ( -file => "/path/configfile.ini" );
...
sub STORE {
  ...
  tie %{$self->{v}{$key}}, 'EB::Config::IniFiles::_section', %parms;
} # end STORE
...
############################################################
#
# INTERNAL PACKAGE: EB::Config::IniFiles::_section
#
# Description:
...
# of a tied hash object with support for the EB::Config::IniFiles
...
############################################################
package EB::Config::IniFiles::_section;

use strict;
use Carp;
use vars qw( $VERSION );

$EB::Config::IniFiles::_section::VERSION = 2.16;

# ----------------------------------------------------------
# Sub: EB::Config::IniFiles::_section::TIEHASH
#
# Args: $class, %parms
...
# ----------------------------------------------------------
# Sub: EB::Config::IniFiles::_section::FETCH
#
# Args: $key
...
# ----------------------------------------------------------
# Sub: EB::Config::IniFiles::_section::STORE
#
# Args: $key, @val
...
# ----------------------------------------------------------
# Sub: EB::Config::IniFiles::_section::DELETE
#
# Args: $key
...
# ----------------------------------------------------------
# Sub: EB::Config::IniFiles::_section::CLEAR
#
# Args: (None)
#
...
# ----------------------------------------------------------
# Sub: EB::Config::IniFiles::_section::EXISTS
#
# Args: $key
...
# ----------------------------------------------------------
# Sub: EB::Config::IniFiles::_section::FIRSTKEY
#
# Args: (None)
#
...
# ----------------------------------------------------------
# Sub: EB::Config::IniFiles::_section::DESTROY
#
# Args: (None)
#
...
if ($^W)	{
#	$EB::Config::IniFiles::VERSION = $EB::Config::IniFiles::VERSION;
}
...
=head2 @EB::Config::IniFiles::errors
